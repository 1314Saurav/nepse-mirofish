  ...
  <div class="report-panel">
    ...
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        ...
        <!-- Error State -->
        <div v-if="!reportOutline && reportError" class="waiting-placeholder error-placeholder">
          <svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="#EF4444" stroke-width="2" style="margin-bottom:12px">
            <circle cx="12" cy="12" r="10"></circle>
            <line x1="12" y1="8" x2="12" y2="12"></line>
            <line x1="12" y1="16" x2="12.01" y2="16"></line>
          </svg>
          <span class="waiting-text" style="color:#EF4444;font-weight:600">Report Generation Failed</span>
          <span class="error-detail">{{ reportError }}</span>
        </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content is ready but the full section may not be complete yet) -->
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: - 选择Parent_601（index 0）：As a parent group representative...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (for old format: only one platform label present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer exists; copy it only when not a placeholder
          ...
          // No platform labels (very old format), use entire text as answer
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 2. "1. " or "\n1. " (number+period, legacy compatibility)
      let matches = []
      let match

      // Try "问题X：" format first (Chinese numbered question format from backend)
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for question 0, empty for the rest
      return qIdx === 0 ? answer : ''
    }

    // Check if a given question has dual-platform answers (filtering placeholder text)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading second-level heading (## xxx) because section title is already shown in outer layer
  ...
  // Remove <p><br> immediately followed by block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> between a closing block-level element and a paragraph opening tag
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: maintain incrementing numbers when single-item <ol>s are separated by paragraph content
    ...
    if (res.success && res.data) {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_error') {
            reportError.value = log.details?.message || log.details?.error || 'Report generation failed'
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" (Chinese final answer label from backend)
