    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing — consider refreshing the graph manually</span>
...
const renderGraph = () => {
  ...
  // Process edge data — count edges and track index for each node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread out when multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      // Distribute curvature evenly for clear visual separation
      // Curvature range increases with edge count — more edges, wider spread
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If the edge direction is reversed relative to canonical order, flip the curvature
      // This ensures all edges are distributed in the same reference frame and do not overlap due to direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center so isolated node clusters converge to the center area
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc that leaves the node and loops back
      const loopRadius = 30
    ...
    // Perpendicular offset from the connecting line, computed proportionally to distance, ensuring the curve is clearly visible
  ...
  // Link label backgrounds (white background makes text more legible)
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // First confirmed drag — restart simulation now
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool down the simulation if an actual drag occurred
        if (d._isDragging) {
