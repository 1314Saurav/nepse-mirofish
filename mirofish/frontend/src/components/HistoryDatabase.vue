        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched while running and do not support simulation playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Currently selected project (used for modal)
let observer = null
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate the number of cards in the current row to ensure each row is centered
    ...
    const x = offset * 35
    // Adjust starting position to be close to the title but maintain appropriate spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update the pending target state (always record the latest target state, even during animation)
        ...
        // If currently animating, only record the state and handle after animation ends
        if (isAnimating) return

        // If the target state is the same as the current state, no action needed
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }

        // Use debounce to delay state switch, preventing rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check if the pending state still needs to be executed (may have been overridden by subsequent scroll)
    ...
    },
    {
      ...
      // Adjust rootMargin, shrink viewport from the bottom upward, requiring more scrolling to trigger expansion
...
.grid-pattern {
  ...
  /* Position from top-left; when height changes, only expands at the bottom without affecting existing grid positions */
