...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect whether the simulation is complete (via runner_status or platform completion flags)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: if the backend hasn't updated runner_status yet but platforms report completion
      // Determined by checking twitter_completed and reddit_completed flags
...
const checkPlatformsCompleted = (data) => {
  ...
  // Determine if a platform is enabled via actions_count (> 0), running flag, or completed flag
...
.timeline-axis {
  ...
  bottom: 0;
  width: 1px;
  background: #EAEAEA; /* Clean axis line */
  transform: translateX(-50%);
}
