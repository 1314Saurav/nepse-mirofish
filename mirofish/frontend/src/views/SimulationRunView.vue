...
// Retrieve maxRounds from query params at initialization so child components get it immediately
...
const handleGoBack = async () => {
  ...
  try {
    // Attempt a graceful shutdown of the simulation environment first
...
const loadSimulationData = async () => {
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      // Fetch simulation config to retrieve minutes_per_round
      try {
...
const loadGraph = async (graphId) => {
  // During active simulation, auto-refresh does not show full-screen loading to avoid flickering.
  // Show loading indicator on manual refresh or initial load.
  if (!isSimulating.value) {
...
const startGraphRefresh = () => {
  ...
  addLog('Graph live refresh enabled (30s interval)')
...
onMounted(() => {
  ...
  // Log maxRounds config (value was already retrieved from query params during initialization)
  if (maxRounds.value) {
